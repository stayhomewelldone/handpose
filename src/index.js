...
const buttonCapture = document.querySelector("#capturefuck")
const buttonCapturewijs = document.querySelector("#capturewijs")
const buttonCapturewanta = document.querySelector("#capturewanta")
...
buttonCapture.addEventListener("click", () => captureFuckyou())
buttonCapturewijs.addEventListener("click", () => capturewijs())
buttonCapturewanta.addEventListener("click", () => capturewanta())
...
async function classify(){
    ...
    let prediction = machine.classify(pose)
    console.log(`I think it's a ${prediction}`)
    if(prediction == "wanta"){

        window.open("https://www.youtube.com/watch?v=vdB-8eLEW8g"); 
    }

}
async function captureFuckyou(){
    const predictions = await model.estimateHands(video)
    console.log(predictions)
    pose = []
    ...
    // const dataArray = Object.values(pose);
    console.log(pose)
    // gebruik de webcam om de posedata uit te lezen
    // let pose = 
    // leer knn dat dit een "standing" pose is
    machine.learn(pose, "fuckyou")
    
}

async function capturewijs(){
    const predictions = await model.estimateHands(video)
    console.log(predictions)
    pose = []
    ...
    // let pose = 
    // leer knn dat dit een "standing" pose is
    machine.learn(pose, "wijsvinger")
    
}

async function capturewanta(){
    const predictions = await model.estimateHands(video)
    console.log(predictions)
    pose = []
